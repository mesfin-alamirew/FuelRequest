...
import { updateFuelPrice, updateVehicle } from '@/lib/actions/admin';
import { Input } from '@/components/ui/Input';
import { Select } from '@/components/ui/Select';
import { FuelPrice, FuelTypeEnum } from '@prisma/client';
import { useActionState } from 'react';

type FormState = { success?: boolean; error?: string } | null;

export default function EditFuelPriceForm({
  fuelPrice,
  onCancel,
  onFuelPriceUpdated,
}: {
  fuelPrice: FuelPrice;
  onCancel: () => void;
  onFuelPriceUpdated: () => Promise<void>;
}) {
  const [state, formAction] = useActionState<FormState, FormData>(
    async (_previousState, formData) => {
      try {
        await updateFuelPrice(fuelPrice.id, formData);
        toast.success('Fuel price updated successfully!');
        await onFuelPriceUpdated();
        onCancel();
        return { success: true };
      } catch (error: unknown) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <h2 className="text-base font-medium text-gray-800 dark:text-white/90">
        Edit Fuel Price: {fuelPrice.type}
      </h2>
      <form action={formAction} className="space-y-4">
        <div className="relative z-20 bg-transparent">
          <Select
            name="fuelType"
            defaultValue={fuelPrice.type}
            required
