...
import { updateCouponValue, updateDriver } from '@/lib/actions/admin';
import { Input } from '@/components/ui/Input';

import { CouponValue, Driver } from '@prisma/client';
import { useActionState } from 'react';

type FormState = { success?: boolean; error?: string } | null;

export default function EditCouponValueForm({
  couponValue,
  onCancel,
  onCouponValueUpdated,
}: {
  couponValue: CouponValue;
  onCancel: () => void;
  onCouponValueUpdated: () => Promise<void>;
}) {
  const [state, formAction] = useActionState<FormState, FormData>(
    async (_previousState, formData) => {
      try {
        await updateCouponValue(couponValue.id, formData);
        toast.success('Coupon value updated successfully!');
        await onCouponValueUpdated();
        onCancel();
        return { success: true };
      } catch (error: unknown) {
        let errorMessage = 'Failed to update coupon value.';
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">
        Edit Coupon Value: {couponValue.value}
      </h2>
      <form action={formAction} className="space-y-4">
        <Input
          name="value"
          placeholder="Coupon Value"
          defaultValue={couponValue.value}
          required
        ...
        <div className="flex gap-2">
          <button
            ...
            // }`}
          >
            {pending ? 'Updating...' : 'Update Coupon Value'}
          </button>
          <button
            type="button"
            onClick={onCancel}
